...
QVector<std::pair<QLineF, QColor>> Detector2DItem::PrepareStrips()
{
    ...
    auto line_uv = [&](int strip_index, bool is_u_strip, float angle) -> QLineF
    {
        ...
        if(is_u_strip)
        {
            ...
            x_intercept = distance / sin(angle_rad);
            y_intercept = distance / cos(angle_rad);
            ...
            if(x_intercept <= data_x_max)
                p1 = Coord(x_intercept, data_y_max);
            else
                p1 = Coord(data_x_max, data_y_max - 
                        (x_intercept-data_x_max) / tan(angle_rad));

            if(y_intercept <= data_y_max)
                p2 = Coord(data_x_min, data_y_max - y_intercept);
            else
                p2 = Coord((y_intercept - data_y_max) * tan(angle_rad), data_y_min);
        }
